...
def find_best_dyson_airstrait_model(user_input: str) -> Optional[str] :
    """
    Ищет модель dyson_airstrait
    ...
    brand_data = PRODUCT_LIBRARY.get("Выпрямители для волос Dyson", {})
    if not brand_data :
        logger.warning("[find_best_dyson_airstrait_model] Нет 'Dyson airstrait' в PRODUCT_LIBRARY.")
        return None
    ...
    for model_name, model_info in brand_data.items() :
        ...
        for alias in aliases :
            ...
            if pattern.search(user_input_lower) :
                # Нашли совпадение
                logger.debug(
                    f"[find_best_dyson_airstrait_model] Совпадение alias='{alias}' -> модель='{model_name}'"
                )
                found_entries.append((model_name, alias))

    if not found_entries :
        logger.info("[find_best_dyson_airstrait_model] Ничего не найдено среди Dyson airstrait-моделей.")
        return None

    # Выберем запись, у которой alias длиннее
    best_model, best_alias = max(found_entries, key=lambda x : len(x[1]))
    logger.info(
        f"[find_best_dyson_airstrait_model] Лучшая модель='{best_model}' (alias='{best_alias}')"
    )
    return best_model


def parse_dyson_airstrait_product(cleaned_line: str, top_level_model: str) -> dict:
    """
    ...
    if top_level_model not in brand_data:
        logger.warning(
            f"[parse_dyson_airstrait_product] Модель '{top_level_model}' нет в 'Dyson airstrait'."
        )
        return {}
...
def handle_dyson_airstrait_product(
    line: str,
    cleaned_line: str,
    model: str,
    countries: list,
    ...
    extracted_attributes = parse_dyson_airstrait_product(cleaned_line, model)
    ...
    if not has_special_alias:
        ...
        required_attrs_mapping = {
            "dyson corrale hs03" : {"color"},
            "dyson corrale hs07" : {"color"},
            "dyson airstrait ht01" : {"color"}
        }
        ...
        logger.info(
            f"[handle_dyson_airstrait_product] Распознано {attr_count} атр.: {recognized_keys} для модели='{model}'"
        )
        ...
        if missing_attrs:
            ...
            logger.info(
                f"[handle_dyson_airstrait_product] Не хватает атрибутов={missing_attrs} для model='{model}' => return False"
            )
            return False

        # Если дошли сюда, значит все обязательные атрибуты присутствуют
        logger.info(
            f"[handle_dyson_airstrait_product] Все обязательные атрибуты ({required_attrs}) найдены для '{model}'."
        )
    ...
    if has_special_alias :
        ...
        logger.info(f"[Dyson airstrait parser] Используем специальное итоговое название: '{final_product_name}'")
    else :
        # Старая логика для Dyson airstrait
        if model.lower().startswith("dyson corrale hs03") :
            ...
            if "model" in extracted_attributes :
                ...
                temp = model.lower().replace("dyson corrale ", "", 1)
                tokens = temp.split()
                if tokens :
        ...
        elif model.lower().startswith("dyson corrale hs07") :
            # Используем модель из extracted_attributes, если она есть
            if "model" in extracted_attributes :
                product_base = extracted_attributes["model"]
            else :
        ...
        elif model.lower().startswith("dyson airstrait ht01") :
            # Используем модель из extracted_attributes, если она есть
            if "model" in extracted_attributes :
                product_base = extracted_attributes["model"]
            else :
                # Если по какой-то причине модели нет в extracted_attributes, удаляем префикс из исходной строки.
                # Делаем это универсально, без привязки к конкретному значению (например, "hs05").
                temp = model.lower().replace("dyson airstrait ", "", 1)
                tokens = temp.split()
                if tokens :
                    first_token = tokens[0]
                    # Если первый токен состоит только из букв и цифр и его длина <= 5, преобразуем его в верхний регистр.
    ...
    if final_product_name is None :
        final_product_name = "Unknown Dyson airstrait"

    logger.info(f"[Dyson airstrait parser] Итоговое наименование сложного товара: '{final_product_name}'")

    # 5. Если страна не указана => ставим ""
    brand = "Выпрямители для волос Dyson"
    if brand == "Выпрямители для волос Dyson" and not countries:
        logger.debug("[Dyson airstrait parser] Не указана страна => ''")
        countries = ["Европа"]
    ...
    logger.debug("[Dyson airstrait parser] Успешно => True")
    return True


def handle_complex_dyson_airstrait(
        line: str,
        cleaned_line: str,
        countries: List[str],
        price: int,
    ...
    """
    Точка входа для обработки Dyson airstrait:
      1) find_best_dyson_airstrait_model — чтобы не «застревать» на коротких алиас
         а собрать все совпадения и выбрать лучший.
      2) handle_dyson_airstrait_product — парсит атрибуты, формирует название, сохраняет.
    """
    best_model = find_best_dyson_airstrait_model(cleaned_line)
    if not best_model :
        logger.info(f"[handle_complex_dyson_airstrait] Не нашли модель Dyson airstrait для '{cleaned_line}'")
        return False
    ...
    logger.info(f"[handle_complex_dyson_airstrait] Лучшая модель Dyson airstrait: '{best_model}'")

    # Теперь парсим атрибуты и сохраняем
    result = handle_dyson_airstrait_product(
        line=line,
