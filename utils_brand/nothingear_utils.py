...
def find_best_nothingear_model(user_input: str) -> Optional[str] :
    """
    Ищет модель Nothing Ear, перебирая все алиасы и отбрасывая кандидатов,
    ...
    """
    brand_data = PRODUCT_LIBRARY.get("Nothing Ear", {})
    if not brand_data :
        logger.warning("[find_best_nothingear_model] Нет 'Nothing Ear' в PRODUCT_LIBRARY.")
        return None
    ...
    if not found_entries :
        logger.info("[find_best_nothingear_model] Ничего не найдено среди Nothing Ear-моделей.")
        return None

    # Выбираем кандидата, у которого алиас наибольшей длины
    best_model, best_alias = max(found_entries, key=lambda x : len(x[1]))
    logger.info(
        f"[find_best_nothingear_model] Лучшая модель='{best_model}' (alias='{best_alias}')"
    )
    return best_model


def parse_nothingear_product(cleaned_line: str, top_level_model: str, line: str) -> dict:
    """
    Парсит дополнительные атрибуты для «Nothing Ear».
    ...
    if top_level_model not in brand_data:
        logger.warning(f"[parse_nothingear_product] Модель '{top_level_model}' нет в 'Nothing Ear'.")
        return {}
    ...
    for attr_name, attr_info in attributes.items():
        ...
        if found_aliases:
            ...
            logger.debug(
                f"[parse_nothingear_product] Для attr='{attr_name}' выбрали value='{best_value}' (alias='{best_alias}', len={len(best_alias)})"
            )
        else:
            logger.debug(f"[parse_nothingear_product] Для attr='{attr_name}' ничего не найдено в строке.")
    ...
    if "color" not in extracted_attributes :
        ...
        if found_colors :
            ...
            logger.debug(
                f"[parse_nothingear_product] Для color выбрали value='{best_color_value}' (alias='{best_color_alias}', len={len(best_color_alias)})"
            )
        else :
            logger.debug(f"[parse_nothingear_product] Для color ничего не найдено в строке.")

    logger.debug(f"[parse_nothingear_product] Итоговые атрибуты: {extracted_attributes}")
    return extracted_attributes



def handle_nothingear_product(
        line: str,
        cleaned_line: str,
        model: str,
        countries: list,
    ...
    extracted_attributes = parse_nothingear_product(cleaned_line, model, line)
    has_special_alias = ("final_product_name" in extracted_attributes)

    if not has_special_alias :
        required_attrs_mapping = {
            "наушники nothing ear" : {"version", "color"},
        }
        required_attrs = required_attrs_mapping.get(model.lower(), set())
        if not required_attrs :
            logger.info(
                f"[handle_nothingear_product] Модель '{model}' не найдена в required_attrs_mapping => return False"
            )
            return False

        recognized_keys = set(extracted_attributes.keys())
        logger.info(
            f"[handle_nothingear_product] Распознано {len(recognized_keys)} атр.: {recognized_keys} для модели='{model}'"
        )
        missing_attrs = required_attrs - recognized_keys
        if missing_attrs :
            logger.info(
                f"[handle_nothingear_product] Не хватает атрибутов={missing_attrs} для model='{model}' => return False"
            )
            return False
        logger.info(
            f"[handle_nothingear_product] Все обязательные атрибуты ({required_attrs}) найдены для '{model}'."
        )
    ...
    if has_special_alias :
        ...
        logger.info(f"[Parser Nothing Ear] Используем специальное итоговое название: '{final_product_name}'")
    else :
        if model.lower().startswith("наушники nothing ear") :
    ...
    if final_product_name is None :
        final_product_name = "Unknown Nothing Ear"

    logger.info(f"[Parser Nothing Ear] Итоговое наименование товара: '{final_product_name}'")

    # Если страна не указана, подставляем пустую строку
    brand = "Nothing Ear"
    if brand == "Nothing Ear" and not countries :
        logger.debug("[Parser Nothing Ear] Не указана страна => ''")
        countries = ["Европа"]
    ...
    logger.debug("[Parser Nothing Ear] Товар успешно обработан => True")
    return True


def handle_complex_nothingear(
        line: str,
        cleaned_line: str,
        countries: List[str],
        price: int,
    ...
    """
    Точка входа для обработки товаров Nothing Ear:
      1) Определяем лучшую модель с учётом игнорируемых слов.
      2) Парсим атрибуты, формируем итоговое название и сохраняем товар.
    """
    best_model = find_best_nothingear_model(cleaned_line)
    if not best_model :
        logger.info(f"[handle_complex_nothingear] Не нашли модель Nothing Ear для '{cleaned_line}'")
        return False
    ...
    if brand in MINIMUM_PRICE_FOR_BRAND :
        ...
        if price < min_price :
            logger.info(
                f"[handle_complex_nothingear] Цена товара ({price}) ниже минимальной ({min_price}) для бренда '{brand}'. Товар не сохраняем."
            )
            return False

    logger.info(f"[handle_complex_nothingear] Лучшая модель Nothing Ear: '{best_model}'")

    result = handle_nothingear_product(
        line=line,
