...
def find_best_meta_model(user_input: str) -> Optional[str] :
    """
    Ищет модель Meta Quest, перебирая все алиасы и отбрасывая кандидатов,
    ...
    """
    brand_data = PRODUCT_LIBRARY.get("Meta Quest", {})
    if not brand_data :
        logger.warning("[find_best_meta_model] Нет 'Meta Quest' в PRODUCT_LIBRARY.")
        return None
    ...
    if not found_entries :
        logger.info("[find_best_meta_model] Ничего не найдено среди Meta Quest-моделей.")
        return None

    # Выбираем кандидата, у которого алиас наибольшей длины
    best_model, best_alias = max(found_entries, key=lambda x : len(x[1]))
    logger.info(
        f"[find_best_meta_model] Лучшая модель='{best_model}' (alias='{best_alias}')"
    )
    return best_model


def parse_meta_product(cleaned_line: str, top_level_model: str, line: str) -> dict:
    """
    Парсит дополнительные атрибуты для «Meta Quest».
    ...
    if top_level_model not in brand_data :
        logger.warning(f"[parse_meta_product] Модель '{top_level_model}' нет в 'Nothing Ear'.")
        return {}
    ...
    for attr_name, attr_info in attributes.items() :
        ...
        if found_aliases :
            ...
            logger.debug(
                f"[parse_meta_product] Для attr='{attr_name}' выбрали value='{best_value}' (alias='{best_alias}', len={len(best_alias)})"
            )
        else :
            logger.debug(f"[parse_meta_product] Для attr='{attr_name}' ничего не найдено в строке.")

    logger.debug(f"[parse_meta_product] Итоговые атрибуты: {extracted_attributes}")
    return extracted_attributes



def handle_meta_product(
        line: str,
        cleaned_line: str,
        model: str,
        countries: list,
    ...
    if not has_special_alias :
        ...
        logger.info(
            f"[handle_meta_product] Распознано {len(recognized_keys)} атр.: {recognized_keys} для модели='{model}'"
        )

        missing_attrs = required_attrs - recognized_keys
        if missing_attrs :
            logger.info(
                f"[handle_meta_product] Не хватает атрибутов={missing_attrs} для model='{model}' => return False"
            )
            return False

        logger.info(
            f"[handle_meta_product] Все обязательные атрибуты ({required_attrs}) найдены для '{model}'."
        )
    ...
    if has_special_alias :
        ...
        logger.info(f"[Parser Meta Quest] Используем специальное итоговое название: '{final_product_name}'")
    else :
        if model.lower().startswith("meta quest") :
    ...
    if final_product_name is None :
        final_product_name = "Unknown Meta Quest"

    logger.info(f"[Parser Meta Quest] Итоговое наименование товара: '{final_product_name}'")

    # Если страна не указана, подставляем пустую строку
    brand = "Meta Quest"
    if brand == "Meta Quest" and not countries :
        logger.debug("[Parser Meta Quest] Не указана страна => ''")
        countries = [""]
    ...
    logger.debug("[Parser Meta Quest] Товар успешно обработан => True")
    return True


def handle_complex_meta(
        line: str,
        cleaned_line: str,
        countries: List[str],
        price: int,
    ...
    """
    Точка входа для обработки товаров Meta Quest:
      1) Определяем лучшую модель с учётом игнорируемых слов.
      2) Парсим атрибуты, формируем итоговое название и сохраняем товар.
    """
    best_model = find_best_meta_model(cleaned_line)
    if not best_model :
        logger.info(f"[handle_complex_meta] Не нашли модель Meta Quest для '{cleaned_line}'")
        return False
    ...
    if brand in MINIMUM_PRICE_FOR_BRAND :
        ...
        if price < min_price :
            logger.info(
                f"[handle_complex_meta] Цена товара ({price}) ниже минимальной ({min_price}) для бренда '{brand}'. Товар не сохраняем."
            )
            return False

    logger.info(f"[handle_complex_meta] Лучшая модель Meta Quest: '{best_model}'")

    result = handle_meta_product(
        line=line,
