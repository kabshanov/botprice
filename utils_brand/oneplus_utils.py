...
from shared_data import (
    PRODUCT_LIBRARY, USER_DATA, ONEPLUS_PATTERN_COLOR, MINIMUM_PRICE_FOR_BRAND,
...
def find_best_oneplus_model(user_input: str) -> Optional[str] :
    """
    ...
    """
    brand_data = PRODUCT_LIBRARY.get("OnePlus", {})
    if not brand_data :
        logger.warning("[find_best_oneplus_model] Нет 'OnePlus' в PRODUCT_LIBRARY.")
        return None
    ...
    for model_name, model_info in brand_data.items() :
        ...
        for alias in aliases :
            ...
            if pattern.search(user_input_lower) :
                ...
                ignoring_words = IGNORING_MODEL_FOR_ONEPLUS.get(model_name, [])
                should_ignore = False
                for word in ignoring_words :
                    ...
                    if ignore_pattern.search(user_input_lower) :
                        logger.debug(
                            f"[find_best_oneplus_model] Пропускаем модель '{model_name}', найдено игнорируемое слово '{word}'"
                        )
                        should_ignore = True
                        break
                if not should_ignore :
                    logger.debug(
                        f"[find_best_oneplus_model] Совпадение alias='{alias}' -> модель='{model_name}'"
                    )
                    found_entries.append((model_name, alias))

    if not found_entries :
        logger.info("[find_best_oneplus_model] Ничего не найдено среди OnePlus-моделей.")
        return None

    # Выбираем кандидата, у которого алиас наибольшей длины
    best_model, best_alias = max(found_entries, key=lambda x : len(x[1]))
    logger.info(
        f"[find_best_oneplus_model] Лучшая модель='{best_model}' (alias='{best_alias}')"
    )
    return best_model


def parse_oneplus_product(cleaned_line: str, top_level_model: str) -> dict:
    """
    ...
    if top_level_model not in brand_data:
        logger.warning(f"[parse_oneplus_product] Модель '{top_level_model}' нет в 'OnePlus'.")
        return {}
    ...
    if match:
        ...
        logger.debug(
            f"[OnePlus Parser] Найден паттерн RAM/SSD: '{match.group(0)}', RAM='{ram_value}', SSD='{ssd_value}'"
        )
    ...
    for attr_name, attr_info in attributes.items():
        ...
        if found_aliases:
            ...
            logger.debug(
                f"[parse_oneplus_product] Для attr='{attr_name}' выбрали value='{best_value}' (alias='{best_alias}', len={len(best_alias)})"
            )
        else:
            logger.debug(f"[parse_oneplus_product] Для attr='{attr_name}' ничего не найдено в строке.")

    if "+ LTE" in extracted_attributes and "Wi-Fi" not in extracted_attributes:
        extracted_attributes["Wi-Fi"] = "Wi-Fi"
        logger.debug(
            "[parse_oneplus_product] 'Cellular' обнаружен, но 'Wi-Fi' отсутствует => добавляем 'Wi-Fi' автоматически."
        )

    # 2) Дополнительный блок для парсинга цвета
    sub_model = extracted_attributes.get("version")
    if sub_model and top_level_model in ONEPLUS_PATTERN_COLOR :
        model_color_info = ONEPLUS_PATTERN_COLOR[top_level_model]
        if sub_model in model_color_info :
            ...
            if found_colors :
                ...
                logger.debug(
                    f"[OnePlus Parser] По OnePlus_PATTERN_COLOR для модели '{top_level_model}' и подмодели '{sub_model}' не найдено совпадений для цвета."
                )
        else :
            logger.debug(
                f"[OnePlus Parser] Для модели '{top_level_model}' отсутствует подмодель '{sub_model}' в OnePlus_PATTERN_COLOR."
            )
    else :
        logger.debug(
            f"[OnePlus Parser] Не найдены данные по цвету: либо sub-model='{sub_model}' отсутствует, либо '{top_level_model}' нет в OnePlus_PATTERN_COLOR."
        )

    logger.debug(f"[parse_oneplus_product] Итоговые атрибуты: {extracted_attributes}")
    return extracted_attributes



def handle_oneplus_product(
        line: str,
        cleaned_line: str,
        model: str,
        countries: list,
    ...
    extracted_attributes = parse_oneplus_product(cleaned_line, model)
    has_special_alias = ("final_product_name" in extracted_attributes)

    if not has_special_alias :
        required_attrs_mapping = {
            "oneplus" : {"version", "RAM", "SSD", "color"},
            "oneplus pad" : {"version", "RAM", "SSD", "color"},
            "oneplus watch" : {"version", "color"},
        }
        required_attrs = required_attrs_mapping.get(model.lower(), set())
        if not required_attrs :
            logger.info(
                f"[handle_oneplus_product] Модель '{model}' не найдена в required_attrs_mapping => return False"
            )
            return False

        recognized_keys = set(extracted_attributes.keys())
        logger.info(
            f"[handle_oneplus_product] Распознано {len(recognized_keys)} атр.: {recognized_keys} для модели='{model}'"
        )
        missing_attrs = required_attrs - recognized_keys
        if missing_attrs :
            logger.info(
                f"[handle_oneplus_product] Не хватает атрибутов={missing_attrs} для model='{model}' => return False"
            )
            return False
        logger.info(
            f"[handle_oneplus_product] Все обязательные атрибуты ({required_attrs}) найдены для '{model}'."
        )
    ...
    if has_special_alias :
        ...
        if model.lower().startswith("oneplus pad") :
            ...
            if "Wi-Fi" in extracted_attributes :
                parts.append(extracted_attributes["Wi-Fi"])
            if "+ LTE" in extracted_attributes :
                parts.append(extracted_attributes["+ LTE"])

            # Собираем финальное название, разделяя части пробелами
            final_product_name = " ".join(parts)

        elif model.lower().startswith("oneplus watch") :
            # Собираем название продукта без использования исходного model
            parts = []
        ...
        elif model.lower().startswith("oneplus") :
            # Собираем название продукта без использования исходного model
            parts = []
    ...
    brand = "OnePlus"
    if brand == "OnePlus" and not countries :
...
def handle_complex_oneplus(
    ...
    """
    best_model = find_best_oneplus_model(cleaned_line)
    if not best_model :
        logger.info(f"[handle_complex_oneplus] Не нашли модель OnePlus для '{cleaned_line}'")
        return False
    ...
    if brand in MINIMUM_PRICE_FOR_BRAND :
        ...
        if price < min_price :
            logger.info(
                f"[handle_complex_oneplus] Цена товара ({price}) ниже минимальной ({min_price}) для бренда '{brand}'. Товар не сохраняем."
            )
            return False

    logger.info(f"[handle_complex_oneplus] Лучшая модель OnePlus: '{best_model}'")

    result = handle_oneplus_product(
        line=line,
