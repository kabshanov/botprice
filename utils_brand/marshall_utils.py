# marshall_utils.py парсим с line
...
def find_best_marshall_model(user_input: str) -> Optional[str] :
    """
    Ищет модель Marshall, перебирая все алиасы и отбрасывая кандидатов,
    ...
    """
    brand_data = PRODUCT_LIBRARY.get("Marshall", {})
    if not brand_data :
        logger.warning("[find_best_marshall_model] Нет 'Marshall' в PRODUCT_LIBRARY.")
        return None
    ...
    for model_name, model_info in brand_data.items() :
        ...
        for alias in aliases :
            ...
            if pattern.search(user_input_lower) :
                # Нашли совпадение
                logger.debug(
                    f"[find_best_marshell_cleaner_model] Совпадение alias='{alias}' -> модель='{model_name}'"
                )
                found_entries.append((model_name, alias))

    if not found_entries :
        logger.info("[find_best_marshall_model] Ничего не найдено среди Marshall-моделей.")
        return None

    # Выбираем кандидата, у которого алиас наибольшей длины
    best_model, best_alias = max(found_entries, key=lambda x : len(x[1]))
    logger.info(
        f"[find_best_marshall_model] Лучшая модель='{best_model}' (alias='{best_alias}')"
    )
    return best_model


def parse_marshall_product(cleaned_line: str, top_level_model: str, line: str) -> dict:
    """
    Парсит дополнительные атрибуты для «Marshall».
    ...
    if top_level_model not in brand_data:
        logger.warning(f"[parse_marshall_product] Модель '{top_level_model}' нет в 'Marshall'.")
        return {}
    ...
    for attr_name, attr_info in attributes.items():
        ...
        if found_aliases:
            ...
            logger.debug(
                f"[parse_marshall_product] Для attr='{attr_name}' выбрали value='{best_value}' (alias='{best_alias}', len={len(best_alias)})"
            )
        else:
            logger.debug(f"[parse_marshall_product] Для attr='{attr_name}' ничего не найдено в строке.")
    ...
    if "color" not in extracted_attributes :
        ...
        if found_colors :
            ...
            logger.debug(
                f"[parse_marshall_product] Для color выбрали value='{best_color_value}' (alias='{best_color_alias}', len={len(best_color_alias)})"
            )
        else :
            logger.debug(f"[parse_marshall_product] Для color ничего не найдено в строке.")


    logger.debug(f"[parse_marshall_product] Итоговые атрибуты: {extracted_attributes}")
    return extracted_attributes



def handle_marshall_product(
        line: str,
        cleaned_line: str,
        model: str,
        countries: list,
    ...
    extracted_attributes = parse_marshall_product(cleaned_line, model, line)
    has_special_alias = ("final_product_name" in extracted_attributes)

    if not has_special_alias :
        required_attrs_mapping = {
            "акустические системы marshall" : {"version", "color"},
            "наушники marshall" : {"version", "color"},
        }
        required_attrs = required_attrs_mapping.get(model.lower(), set())
        if not required_attrs :
            logger.info(
                f"[handle_marshall_product] Модель '{model}' не найдена в required_attrs_mapping => return False"
            )
            return False
        ...
        logger.info(
            f"[handle_marshall_product] Распознано {len(recognized_keys)} атр.: {recognized_keys} для модели='{model}'"
        )
        missing_attrs = required_attrs - recognized_keys
        if missing_attrs :
            logger.info(
                f"[handle_marshall_product] Не хватает атрибутов={missing_attrs} для model='{model}' => return False"
            )
            return False
        logger.info(
            f"[handle_marshall_product] Все обязательные атрибуты ({required_attrs}) найдены для '{model}'."
        )
    ...
    if has_special_alias :
        ...
        logger.info(f"[Parser Marshall] Используем специальное итоговое название: '{final_product_name}'")
    else :
        if model.lower().startswith("акустические системы marshall") :
        ...
        elif model.lower().startswith("наушники marshall") :
            # Собираем название продукта без использования исходного model
            parts = []
    ...
    if final_product_name is None :
        final_product_name = "Unknown Marshall"

    logger.info(f"[Parser Marshall] Итоговое наименование товара: '{final_product_name}'")

    # Если страна не указана, подставляем пустую строку
    brand = "Marshall"
    if brand == "Marshall" and not countries :
        logger.debug("[Parser Marshall] Не указана страна => ''")
        countries = ["Европа"]
    ...
    logger.debug("[Parser Marshall] Товар успешно обработан => True")
    return True


def handle_complex_marshall(
        line: str,
        cleaned_line: str,
        countries: List[str],
        price: int,
    ...
    """
    Точка входа для обработки товаров Marshall:
      1) Определяем лучшую модель с учётом игнорируемых слов.
      2) Парсим атрибуты, формируем итоговое название и сохраняем товар.
    """
    best_model = find_best_marshall_model(cleaned_line)
    if not best_model :
        logger.info(f"[handle_complex_marshall] Не нашли модель Marshall для '{cleaned_line}'")
        return False
    ...
    if brand in MINIMUM_PRICE_FOR_BRAND :
        ...
        if price < min_price :
            logger.info(
                f"[handle_complex_marshall] Цена товара ({price}) ниже минимальной ({min_price}) для бренда '{brand}'. Товар не сохраняем."
            )
            return False

    logger.info(f"[handle_complex_marshall] Лучшая модель Marshall: '{best_model}'")

    result = handle_marshall_product(
        line=line,
