...
from shared_data import (
    PRODUCT_LIBRARY, USER_DATA, REALME_PATTERN_COLOR, MINIMUM_PRICE_FOR_BRAND,
...
def find_best_realme_model(user_input: str) -> Optional[str] :
    """
    Ищет модель Realme, перебирая все алиасы и отбрасывая кандидатов,
    ...
    """
    brand_data = PRODUCT_LIBRARY.get("Realme", {})
    if not brand_data :
        logger.warning("[find_best_realme_model] Нет 'Realme' в PRODUCT_LIBRARY.")
        return None

    user_input_lower = user_input.lower()
    found_entries = []  # Список кортежей (model_name, alias)

    # Перебираем модели в PRODUCT_LIBRARY для Realme
    for model_name, model_info in brand_data.items() :
        ...
        for alias in aliases :
            ...
            if pattern.search(user_input_lower) :
                ...
                ignoring_words = IGNORING_MODEL_FOR_REALME.get(model_name, [])
                should_ignore = False
                for word in ignoring_words :
                    ...
                    if ignore_pattern.search(user_input_lower) :
                        logger.debug(
                            f"[find_best_realme_model] Пропускаем модель '{model_name}', найдено игнорируемое слово '{word}'"
                        )
                        should_ignore = True
                        break
                if not should_ignore :
                    logger.debug(
                        f"[find_best_realme_model] Совпадение alias='{alias}' -> модель='{model_name}'"
                    )
                    found_entries.append((model_name, alias))

    if not found_entries :
        logger.info("[find_best_realme_model] Ничего не найдено среди Realme-моделей.")
        return None

    # Выбираем кандидата, у которого алиас наибольшей длины
    best_model, best_alias = max(found_entries, key=lambda x : len(x[1]))
    logger.info(
        f"[find_best_realme_model] Лучшая модель='{best_model}' (alias='{best_alias}')"
    )
    return best_model


def parse_realme_product(cleaned_line: str, top_level_model: str) -> dict:
    """
    Парсит дополнительные атрибуты для «Realme».
    ...
    if top_level_model not in brand_data:
        logger.warning(f"[parse_realme_product] Модель '{top_level_model}' нет в 'Realme'.")
        return {}
    ...
    if match:
        ...
        logger.debug(
            f"[Realme Parser] Найден паттерн RAM/SSD: '{match.group(0)}', RAM='{ram_value}', SSD='{ssd_value}'"
        )
    ...
    for attr_name, attr_info in attributes.items():
        ...
        if found_aliases:
            ...
            logger.debug(
                f"[parse_realme_product] Для attr='{attr_name}' выбрали value='{best_value}' (alias='{best_alias}', len={len(best_alias)})"
            )
        else:
            logger.debug(f"[parse_realme_product] Для attr='{attr_name}' ничего не найдено в строке.")

    # 2) Дополнительный блок для парсинга цвета
    sub_model = extracted_attributes.get("version")
    if sub_model and top_level_model in REALME_PATTERN_COLOR :
        model_color_info = REALME_PATTERN_COLOR[top_level_model]
        if sub_model in model_color_info :
            ...
            if found_colors :
                ...
                logger.debug(
                    f"[Realme Parser] Для 'color' выбрали '{best_color}' (найден алиас '{best_alias}', len={len(best_alias)})"
                )
            else :
                logger.debug(
                    f"[Realme Parser] По REALME_PATTERN_COLOR для модели '{top_level_model}' и подмодели '{sub_model}' не найдено совпадений для цвета."
                )
        else :
            logger.debug(
                f"[Realme Parser] Для модели '{top_level_model}' отсутствует подмодель '{sub_model}' в REALME_PATTERN_COLOR."
            )
    else :
        logger.debug(
            f"[Realme Parser] Не найдены данные по цвету: либо sub-model='{sub_model}' отсутствует, либо '{top_level_model}' нет в REALME_PATTERN_COLOR."
        )

    logger.debug(f"[parse_realme_product] Итоговые атрибуты: {extracted_attributes}")
    return extracted_attributes



def handle_realme_product(
        line: str,
        cleaned_line: str,
        model: str,
        countries: list,
    ...
    extracted_attributes = parse_realme_product(cleaned_line, model)
    has_special_alias = ("final_product_name" in extracted_attributes)

    if not has_special_alias :
        required_attrs_mapping = {
            "realme" : {"version", "RAM", "SSD", "color"},
        }
        required_attrs = required_attrs_mapping.get(model.lower(), set())
        if not required_attrs :
            logger.info(
                f"[handle_realme_product] Модель '{model}' не найдена в required_attrs_mapping => return False"
            )
            return False

        recognized_keys = set(extracted_attributes.keys())
        logger.info(
            f"[handle_realme_product] Распознано {len(recognized_keys)} атр.: {recognized_keys} для модели='{model}'"
        )
        missing_attrs = required_attrs - recognized_keys
        if missing_attrs :
            logger.info(
                f"[handle_realme_product] Не хватает атрибутов={missing_attrs} для model='{model}' => return False"
            )
            return False
        logger.info(
            f"[handle_realme_product] Все обязательные атрибуты ({required_attrs}) найдены для '{model}'."
        )
    ...
    if has_special_alias :
        ...
        logger.info(f"[Realme Parser] Используем специальное итоговое название: '{final_product_name}'")
    else :
    ...
    if final_product_name is None :
        final_product_name = "Unknown Realme"

    logger.info(f"[Realme Parser] Итоговое наименование товара: '{final_product_name}'")

    # Если страна не указана, подставляем пустую строку
    brand = "Realme"
    if brand == "Realme" and not countries :
        logger.debug("[Realme Parser] Не указана страна => ''")
        countries = [""]
    ...
    logger.debug("[Realme Parser] Товар успешно обработан => True")
    return True


def handle_complex_realme(
        line: str,
        cleaned_line: str,
        countries: List[str],
        price: int,
    ...
    """
    Точка входа для обработки товаров Realme:
      1) Определяем лучшую модель с учётом игнорируемых слов.
      2) Парсим атрибуты, формируем итоговое название и сохраняем товар.
    """
    best_model = find_best_realme_model(cleaned_line)
    if not best_model :
        logger.info(f"[handle_complex_realme] Не нашли модель Realme для '{cleaned_line}'")
        return False
    ...
    if brand in MINIMUM_PRICE_FOR_BRAND :
        ...
        if price < min_price :
            logger.info(
                f"[handle_complex_realme] Цена товара ({price}) ниже минимальной ({min_price}) для бренда '{brand}'. Товар не сохраняем."
            )
            return False

    logger.info(f"[handle_complex_realme] Лучшая модель Realme: '{best_model}'")

    result = handle_realme_product(
        line=line,
