...
from shared_data import (
    ...
    COUNTRY_WORD_MAP, COMMENT_ADD, IGNORING_COMMENT, CODES_NO_PRICE
)
...
def is_multiline_price_message(text: str) -> bool:
    """
    True, если в тексте есть ровно 'от 1' (в разных вариациях):
      - от 1
      - от1
      - от 1 шт / (от 1 шт)
    Любые другие числа (от 2, от 10…) игнорируются.
    """
    pattern = r'(?i)\(?от\s*1(?!\d)\s*(?:шт\.?)?\)?'   # <‑‑ отриц. просмотр (?!\d) запрещает 2‑ю цифру
...
def parse_multiline_price(text: str) :
    ...
    pattern_from1 = re.compile(
        r'(?i)\(?\s*от\s*1\s*(?:шт\.?)?\)?'  # допускаем скобки и «шт»
        r'\s*[-–—>]+\s*'  # тире / стрелки
        r'(?P<price>\d[\d\s.,]*)'  # сама цена (в группу ‹price›)
    )
...
def normalize_product_name(product_name: str) -> str:
    ...
    mapping = {
        ...
        'jblclip' : 'jbl clip',
        'jblflip' : 'jbl flip',
        'jblcharge' : 'jbl charge',
        'jblxtreme' : 'jbl xtreme',
        'jblboombox' : 'jbl boombox',
        'jblpartybox' : 'jbl partybox',
        'jbl party box' : 'jbl partybox',
        'jblpulse' : 'jbl pulse',
        'jblwind' : 'jbl wind',
        'jbllive' : 'jbl live',
        'jbl детские' : 'jbl jr',
        ...
        'stem deck' : 'steam deck',
        'stemdeck' : 'steam deck',
        'A.N.C.' : 'ANC',

        # Перевод gb и гб в числовые значения
        '64gb' : '64',
        '64 gb' : '64',
        '64гб' : '64',
        '64 гб' : '64',
...
def extract_data(line: str) -> Tuple[Optional[str], Optional[int], str]:
    ...
    try:
        ...
        # -----------------------------------------------------------------------------
        # 3‑a. Фильтр «артикулов» — исключаем кандидаты из списка CODES_NO_PRICE
        #      (список расположен в shared_data.py и импортируется как CODES_NO_PRICE)
        # -----------------------------------------------------------------------------
        def _digits_only(s: str) -> str:
            # Приводим строку к одной «цифровой» форме: '470 521' → '470521'
            return re.sub(r"\D", "", s)

        parsed_prices = [
            (s, v) for (s, v) in parsed_prices
            if _digits_only(s) not in CODES_NO_PRICE
        ]
...
def handle_complex_brand(
    ...
    for brand_candidate in brand_candidates:
        ...
        if brand_candidate == "Apple iPhone":
            from utils_brand.apple_iphone_utils import handle_complex_apple_iphone
        ...
        elif brand_candidate == "Apple Watch":
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            from utils_brand.apple_watch_utils import handle_complex_apple_watch
            logger.debug("[handle_complex_brand] Вызываем handle_complex_apple_watch...")
            handled = handle_complex_apple_watch(
                line=line,
                cleaned_line=cleaned_line,
                countries=countries,
                price=price,
            ...
            logger.debug(f"[handle_complex_brand] handle_complex_apple_watch вернул {handled}")
            if handled:
                return True  # Возвращаем результат вызова функции

        elif brand_candidate == "Apple Mac":
            from utils_brand.apple_mac_utils import handle_complex_apple_mac        # Новый блок для Mac
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            logger.debug("[handle_complex_brand] Вызываем handle_complex_apple_mac...")
            handled = handle_complex_apple_mac(
                line=line,
                cleaned_line=cleaned_line,
                countries=countries,
                price=price,
            ...
            logger.debug(f"[handle_complex_brand] handle_complex_apple_mac вернул {handled}")
            if handled:
                return True  # Возвращаем результат вызова функции

        elif brand_candidate == "Apple Airpods":
            from utils_brand.apple_airpods_utils import handle_complex_apple_airpods
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            logger.debug("[handle_complex_apple_airpods] Вызываем handle_complex_apple_airpods...")
            handled = handle_complex_apple_airpods(
                line=line,
                cleaned_line=cleaned_line,
                countries=countries,
                price=price,
            ...
            logger.debug(f"[handle_complex_brand] handle_complex_apple_airpods вернул {handled}")
            if handled:
                return True  # Возвращаем результат вызова функции

        elif brand_candidate == "Apple iPad":
            from utils_brand.apple_ipad_utils import handle_complex_apple_ipad
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            logger.debug("[handle_complex_apple_ipad] Вызываем handle_complex_apple_ipad...")
            handled = handle_complex_apple_ipad(
                line=line,
                cleaned_line=cleaned_line,
                countries=countries,
                price=price,
            ...
            logger.debug(f"[handle_complex_brand] handle_complex_apple_ipad вернул {handled}")
            if handled:
                return True  # Возвращаем результат вызова функции

        elif brand_candidate == "Apple Аксессуары":
            from utils_brand.apple_accessories_utils import handle_complex_apple_accessories
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.samsung_galaxy_utils import handle_complex_samsung_galaxy
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.dyson_airwrap_utils import handle_complex_dyson_airwrap
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.dyson_supersonic_utils import handle_complex_dyson_supersonic
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.dyson_airstrait_utils import handle_complex_dyson_airstrait
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.dyson_cleaner_utils import handle_complex_dyson_cleaner
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.dyson_climate_utils import handle_complex_dyson_climate
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.dyson_handdryer_utils import handle_complex_dyson_handdryer
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.dyson_accessories_utils import handle_complex_dyson_accessories
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.xiaomi_utils import handle_complex_xiaomi
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.oneplus_utils import handle_complex_oneplus
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.google_utils import handle_complex_google
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.realme_utils import handle_complex_realme
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.honor_utils import handle_complex_honor
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.infinix_utils import handle_complex_infinix
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.doogee_utils import handle_complex_doogee
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.yandex_utils import handle_complex_yandex
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.jbl_utils import handle_complex_jbl
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.beats_utils import handle_complex_beats
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.sonyaudio_utils import handle_complex_sonyaudio
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.marshall_utils import handle_complex_marshall
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.galaxybuds_utils import handle_complex_galaxybuds
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.oneplusbuds_utils import handle_complex_oneplusbuds
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.redmibuds_utils import handle_complex_redmibuds
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.nothingear_utils import handle_complex_nothingear
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            logger.debug("[handle_complex_oneplus] Вызываем handle_complex_oneplus...")
            ...
            from utils_brand.vk_utils import handle_complex_vk
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.audio_utils import handle_complex_audio
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.xbox_utils import handle_complex_xbox
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.playstation_utils import handle_complex_playstation
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.meta_utils import handle_complex_meta
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.steamdeck_utils import handle_complex_steamdeck
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            ...
            from utils_brand.nintendo_utils import handle_complex_nintendo
            # "Ленивый" импорт, чтобы избежать циклических импортов.
        ...
        elif brand_candidate == "GoPro":
            from utils_brand.gopro_utils import handle_complex_gopro
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            logger.debug("[handle_complex_gopro] Вызываем handle_complex_gopro...")
            handled = handle_complex_gopro(
                line=line,
                cleaned_line=cleaned_line,
                countries=countries,
                price=price,
            ...
            logger.debug(f"[handle_complex_brand] handle_complex_gopro вернул {handled}")
            if handled:
                return True  # Возвращаем результат вызова функции
            ...
            # "Ленивый" импорт, чтобы избежать циклических импортов.
            logger.debug("[handle_complex_tecno] Вызываем handle_complex_tecno...")
            handled = handle_complex_tecno(
                line=line,
                cleaned_line=cleaned_line,
                countries=countries,
                price=price,
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    async def process_single_line(original_line: str, line: str) -> bool :
        ...
        #     • Удаляем страновые флаги из comment_text, чтобы не дублировать их
        #     • Передаём в process_emojis список ALLOWED_COMMENT_EMOJIS без этих флагов
        for fl in flags :  # flags получили выше через extract_flags
            comment_text = comment_text.replace(fl, "").strip()

        allowed_no_flags = [e for e in ALLOWED_COMMENT_EMOJIS if e not in flags]
    ...
    try:
        ...
        # ------------------------------------------------------------------
        # МНОГОСТРОЧНЫЙ ПРАЙС «От1 – …»
        # ------------------------------------------------------------------
        if is_multiline_price_message(user_message) :
            logger.info("✅ Многострочный режим")

            supplier = extract_supplier(update) or ""  # ① для process_single_line
            lines_lst = user_message.splitlines()  # ★ понадобится далее

            # --- 1. сначала обрабатываем позиции «от 1» ---------------------
            items = parse_multiline_price(user_message)
            processed_idx: set[int] = set()

            for item in items :
                synthetic = f"{item['name']} - {item['price']}"
                await process_single_line(synthetic, synthetic)

                # помечаем строки, которые «съели», чтобы не дублировать
                for i, ln in enumerate(lines_lst) :
                    if synthetic.split(' - ')[0].strip() in ln :
                        processed_idx.add(i)

            # --- вспомогательная функция: ищем предыдущую товарную строку ---
            def find_prev_product(idx: int) -> str | None :  # ★
                for j in range(idx - 1, -1, -1) :
                    if j in processed_idx :
                        # даже если строка уже «съедена», она может быть товаром
                        ln = lines_lst[j].strip()
                        if ln and not re.search(r'\bот\s+\d+', ln, re.I) :
                            return ln
                    else :
                        ln = lines_lst[j].strip()
                        if ln and not re.search(r'\bот\s+\d+', ln, re.I) :
            ...
            # --- 2. добираем оставшиеся строки ------------------------------
            singles_added: list[str] = []
            current_product_line: str | None = None

            for i, raw in enumerate(lines_lst) :
                stripped = raw.strip()
                if i in processed_idx or not stripped :
                    continue

                # 2.1 если строка – “товар” (нет «от N»), запоминаем её
                if not re.search(r'\bот\s+\d+', stripped, re.I) :
                    current_product_line = stripped

                ok = await process_single_line(stripped, stripped)

                # 2.2 формируем человекочитаемый вывод
                if ok :
                    if re.search(r'\bот\s+\d+', stripped, re.I) :  # «От N»‑хвост
                        prod = current_product_line or find_prev_product(i)  # ★
                        if prod :
                            singles_added.append(f"{prod}  →  {stripped}")
                        else :
                            singles_added.append(stripped)
                    else :
                        singles_added.append(stripped)

            # --- 3. итоговый ответ пользователю ------------------------------
            msg = [f"Добавлено (мультиформат): {recognized_count} товаров."]
            if singles_added :
                msg.append("")
                msg.append("↳ Также распознаны многострочные позиции:")
                msg += [f"• {l}" for l in singles_added[:10]]
                if len(singles_added) > 10 :
                    msg.append("…")
